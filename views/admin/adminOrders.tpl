<h2>{$pageTitle}/Раздел Заказов</h2>
{if ! $rsOrders}
<p>Нет заказов</p>
{else}
<table border="1" cellpadding="1" cellspacing="1">
	...
	<tr>
		...
		<td><a href="#"  id="itemLink_{$item['id']}" onclick="showProducts('{$item['id']}'); return false;">Показать товар заказа</a></td>
		<td>{$item['id']}</td>
		<td>
			<input type="checkbox" id="itemStatus_{$item['id']}" {if $item['status']} checked="checked" {/if} onclick="updateOrderStatus('{$item['id']}');" /><span name="itemStatus_{$item['id']}">{if $item['status']} Закрыто {else} Открыто {/if}</span>
		</td>
